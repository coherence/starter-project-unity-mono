...
namespace Coherence.Generated.Internal
{
	...
	public class DetectChangedComponentsSystem : SystemBase
	{
		...
		protected override void OnUpdate()
		{
			...
			Entities.ForEach((Entity entity, ref Cube_Cube_Sync sync, in global::Coherence.Generated.Cube_Cube data, in Simulated simulate) =>
			{
				if (time - sync.lastSampledTime < sync.minSamplingTime)
				{
					return;
				...
				if (data.friend != sync.lastSentData.friend) mask |= 0b00000000000000000000000000000001;
				...
				{
					entity = entity,
					componentType = TypeIds.InternalCube_Cube,
					state = sync.hasReceivedConstructor ? ComponentState.Update : ComponentState.Construct,
					priority = (int)sync.howImportantAreYou,
					mask = mask,
					simulationFrame = simulationFrame,
			...
			Entities.ForEach((Entity entity, ref Player_Controller_Sync sync, in global::Coherence.Generated.Player_Controller data, in Simulated simulate) =>
			{
				if (time - sync.lastSampledTime < sync.minSamplingTime)
				{
					return;
				...
				if (data.otherPlayer != sync.lastSentData.otherPlayer) mask |= 0b00000000000000000000000000000001;

				if (data.otherPlayerTransform != sync.lastSentData.otherPlayerTransform) mask |= 0b00000000000000000000000000000010;

				if (data.otherPlayerSync != sync.lastSentData.otherPlayerSync) mask |= 0b00000000000000000000000000000100;
				...
				{
					entity = entity,
					componentType = TypeIds.InternalPlayer_Controller,
					state = sync.hasReceivedConstructor ? ComponentState.Update : ComponentState.Construct,
					priority = (int)sync.howImportantAreYou,
					mask = mask,
					simulationFrame = simulationFrame,
