...
namespace Coherence.Generated
{
	...
	public class EntityReplacer {
		...
		private static void CopyComponents(EntityManager entityManager, Entity source, Entity destination)
		{
			...
			if (entityManager.HasComponent<Cube_Cube>(source))
			{ 
				// Cube_Cube has fields, will copy it.			
				if (!entityManager.HasComponent<Cube_Cube>(destination)) {
					entityManager.AddComponentData<Cube_Cube>(destination, new Cube_Cube());
				}
				var data = entityManager.GetComponentData<Cube_Cube>(source);
				entityManager.SetComponentData<Cube_Cube>(destination, data);
			}

			if (entityManager.HasComponent<Player_Controller>(source))
			{ 
				// Player_Controller has fields, will copy it.			
				if (!entityManager.HasComponent<Player_Controller>(destination)) {
					entityManager.AddComponentData<Player_Controller>(destination, new Player_Controller());
				}
				var data = entityManager.GetComponentData<Player_Controller>(source);
				entityManager.SetComponentData<Player_Controller>(destination, data);
			}
			...
			if (entityManager.HasComponent<Player_Controller_Foo>(source) &&
				!entityManager.HasComponent<Player_Controller_Foo>(destination)) {
				entityManager.AddBuffer<Player_Controller_Foo>(destination);
				entityManager.GetBuffer<Player_Controller_Foo>(destination).CopyFrom(
					entityManager.GetBuffer<Player_Controller_Foo>(source));
			}			
			if (entityManager.HasComponent<Player_Controller_FooRequest>(source) &&
				!entityManager.HasComponent<Player_Controller_FooRequest>(destination)) {
				entityManager.AddBuffer<Player_Controller_FooRequest>(destination);
