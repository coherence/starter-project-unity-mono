	...
	public class ReceiveUpdate : IReceiveUpdate
	{
		...
		private void DestroyComponentData(EntityManager entityManager, Entity entity, uint componentType)
		{
			switch (componentType)
			{
				...
				case TypeIds.InternalCube_Cube:
				{
					...
					if (hasComponentData)
					{
						entityManager.RemoveComponent<Cube_Cube>(entity);
					}
					break;
				}
				case TypeIds.InternalPlayer_Controller:
				{
					...
					if (hasComponentData)
					{
						entityManager.RemoveComponent<Player_Controller>(entity);
					}
					break;
				}
				default:
		...
		public void UpdateHasReceivedConstructor(EntityManager entityManager, Entity entity, uint componentTypeId)
		{
			...
			switch (componentTypeId)
			{
				...
				case TypeIds.InternalCube_Cube:
				{
					var hasComponentData = entityManager.HasComponent<Cube_Cube_Sync>(entity);
					if (hasComponentData)
					{
						var syncData = entityManager.GetComponentData<Cube_Cube_Sync>(entity);
						syncData.hasReceivedConstructor = true;
						entityManager.SetComponentData(entity, syncData);
					} else
					{
						// Ownership may have been lost since the packet was sent
						Log.Trace($"Sync component has been destroyed: {entity} Cube_Cube_Sync");
					}
					break;
				}
				case TypeIds.InternalPlayer_Controller:
				{
					var hasComponentData = entityManager.HasComponent<Player_Controller_Sync>(entity);
					if (hasComponentData)
					{
						var syncData = entityManager.GetComponentData<Player_Controller_Sync>(entity);
						syncData.hasReceivedConstructor = true;
						entityManager.SetComponentData(entity, syncData);
					} else
					{
						// Ownership may have been lost since the packet was sent
						Log.Trace($"Sync component has been destroyed: {entity} Player_Controller_Sync");
					}
					break;
				}
				default:
		...
		public static void RemoveSyncComponents(EntityManager entityManager, Entity entity)
		{
			...
			if (entityManager.HasComponent<Cube_Cube_Sync>(entity))
			{
				entityManager.RemoveComponent<Cube_Cube_Sync>(entity);
			}
			if (entityManager.HasComponent<Player_Controller_Sync>(entity))
			{
				entityManager.RemoveComponent<Player_Controller_Sync>(entity);
			}
		}
		...
		public static void AddCleanSyncComponents(EntityManager entityManager, Entity entity)
		{
			...
			if (entityManager.HasComponent<Cube_Cube>(entity))
			{
				entityManager.AddComponentData(entity, new Cube_Cube_Sync
				{
					howImportantAreYou = 100,
					hasBeenSerialized = true,
					lastSentData = entityManager.GetComponentData<Cube_Cube>(entity)
				});
			}
			if (entityManager.HasComponent<Player_Controller>(entity))
			{
				entityManager.AddComponentData(entity, new Player_Controller_Sync
				{
					howImportantAreYou = 100,
					hasBeenSerialized = true,
					lastSentData = entityManager.GetComponentData<Player_Controller>(entity)
				});
			}
		}
