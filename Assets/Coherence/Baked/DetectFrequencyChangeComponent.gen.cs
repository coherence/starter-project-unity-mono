...
namespace Coherence.Generated.Internal
{
	...
	public class DetectFrequencyComponentsSystem : SystemBase
	{
		...
		protected override void OnUpdate()
		{
			...
			Entities
			.ForEach((Entity entity, int entityInQueryIndex, ref Cube_Cube_Sync sync, in SetCube_CubeFrequency data) =>
			{
				sync.minSamplingTime = 1/data.samplingFrequency;
				ecb.RemoveComponent<SetCube_CubeFrequency>(entityInQueryIndex, entity);
			}).ScheduleParallel();
			Entities
			.ForEach((Entity entity, int entityInQueryIndex, ref Player_Controller_Sync sync, in SetPlayer_ControllerFrequency data) =>
			{
				sync.minSamplingTime = 1/data.samplingFrequency;
				ecb.RemoveComponent<SetPlayer_ControllerFrequency>(entityInQueryIndex, entity);
			}).ScheduleParallel();
