	...
	public class PerformCommands : IPerformCommand
	{
		...
		public void PerformCommand(EntityManager mgr, Entity entity, uint commandTypeID, Coherence.Replication.Protocol.Definition.IInBitStream bitStream)
		{
			...
			switch (commandTypeID)
			{
				...
				case TypeIds.InternalPlayer_Controller_Blaj:
				{
					var data = new Player_Controller_Blaj();
					messageDeserializers.Player_Controller_Blaj(bitStream, ref data);

					if (mgr.Exists(entity))
					{
						var hasRequestBuffer = mgr.HasComponent<Player_Controller_Blaj>(entity);
						if (!hasRequestBuffer)
						{
							mgr.AddBuffer<Player_Controller_Blaj>(entity);
						}
						var buffer = mgr.GetBuffer<Player_Controller_Blaj>(entity);					
						buffer.Add(data);
					}
					break;
				}
